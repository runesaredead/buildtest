...
const leaderboard = {
    ...
    draw: function() {
        // Debug log to check if draw is being called
        console.log('Drawing leaderboard, visible:', this.visible);
        
        // Draw button in home state
        if(state.current == state.home) {
            console.log('Drawing leaderboard button');
            ctx.drawImage(
                sprite_sheet,
        ...
        if(this.visible) {
            console.log('Drawing leaderboard UI');
            // Draw background
            ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';  // Semi-transparent black
            ctx.fillRect(this.board.x, this.board.y, this.board.w, this.board.h);

            // Draw title
            ctx.fillStyle = "#FFF";
            ctx.font = "20px Teko";
            ctx.textAlign = "center";  // Add this to center text
